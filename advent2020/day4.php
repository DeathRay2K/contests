...
function isValid( $passport )
{
	...
	if( $passport['byr'] < 1920 || $passport['byr'] > 2002 || strlen( $passport['byr'] ) !== 4 )
		return false;

	if( $passport['iyr'] < 2010 || $passport['iyr'] > 2020 || strlen( $passport['iyr'] ) !== 4 )
		return false;
	
	if( $passport['eyr'] < 2020 || $passport['eyr'] > 2030 || strlen( $passport['eyr'] ) !== 4 )
		return false;

	$hgtUnit = substr($passport['hgt'], -2);
	switch( $hgtUnit )
	{
		case 'cm':
	...
	if( $passport['hcl'][0] !== '#' || strlen($passport['hcl']) !== 7 )
		return false;

	foreach(str_split(substr($passport['hcl'],1)) as $ch)
		if(strpos('1234567890qwertyuiopasdfghjklzxcvbnm', $ch) === false)
			return false;
...
while( $i < count( $inputLines ) )
{
	$line = $inputLines[$i];
	if( $inputLines[$i] == "" )
	{
		$passports[] = $currPassport;
		$currPassport = [];
		$i++;
		continue;
	}
	...
	foreach($fields as $field)
	{
		...
		$currPassport[$fieldKey] = $fieldValue;
	}
	$i++;
}
$passports[] = $currPassport;
