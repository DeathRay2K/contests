<?php
$input = 'onitvpuwxqczlmkabsjrgedfy
...
function partTwo( $inputLines )
{
	...
	$sumQs = array_reduce($groups, function($sum, $group){
		$ppl = $group['ppl'];
		unset($group['ppl']);
		foreach($group as $q => $num)
			if($num == $ppl)
				$sum++;
